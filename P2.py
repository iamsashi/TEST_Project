...
# ================= 1. RE-TUNED SENSITIVITY (RELAXED) =================
MOVEMENT_THRESHOLD = 4        
INSTANT_MOVE_LIMIT = 25       

# Talking: Increased to 5.0 (Must speak clearly to trigger)
TALKING_THRESHOLD = 5.0       

# Gaze Limits: Widened the "Safe Zone"
# Students must turn head FURTHER to trigger alert
GAZE_LEFT_LIMIT = 1.25   
GAZE_RIGHT_LIMIT = 0.75
GAZE_UP_LIMIT = 0.35
GAZE_DOWN_LIMIT = 1.50
    ...
    def update(self, detections):
        ...
        for obj_id, idx, dist in matches:
            ...
            if np.var(list(self.mouth_history[obj_id])) > TALKING_THRESHOLD:
                current_status = "INATTENTIVE (Talking)" 
            
            # Specific Gaze Directions
            elif h_ratio < GAZE_RIGHT_LIMIT: 
                current_status = "DISTRACTED (Looking Right)"
            elif h_ratio > GAZE_LEFT_LIMIT: 
                current_status = "DISTRACTED (Looking Left)"
            elif v_ratio < GAZE_UP_LIMIT: 
                current_status = "DISTRACTED (Looking Up)"
            elif v_ratio > GAZE_DOWN_LIMIT: 
                current_status = "DISTRACTED (Looking Down)"
            
            # Movement Priority
            elif score > 100:
                current_status = "INATTENTIVE (Moving)" 
    ...
    try:
        while cap.isOpened():
            ...
            for obj_id, data in tracker.objects.items():
                ...
                cv2.putText(frame, status, (cx - 70, cy - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
